...
export const ExpenseFormModal = ({
  ...
  const [formState, setFormState] = useState<ExpenseFormValues>(() =>
    buildExpenseFormValues(initialExpense),
  );
  ...
  const remaining = selectedBudget
    ? Math.max(selectedBudget.amount - (selectedBudget.spendAmount ?? 0), 0)
    : null;
  ...
  const isDateValid = !formState.date || formState.date <= dateConstraints.max;
  ...
  const findMatchingBudget = (
    ...
    if (categoryId) {
      const categoryBudgets = budgets.filter(
        (budget) =>
          budget.category.id === categoryId || budget.categoryId === categoryId,
      );

      return (
        categoryBudgets.find(
          (budget) =>
            Math.max(budget.amount - (budget.spendAmount ?? 0), 0) > 0,
        ) ??
        categoryBudgets[0] ??
        null
      );
    ...
    if (categoryName) {
      ...
      return (
        budgets.find(
          (budget) =>
            budget.category.name.trim().toLowerCase() ===
  ...
  const handleScanComplete = (scannedData: ReceiptScanResult) => {
    ...
    const scannedAmount =
      typeof scannedData.amount === "number"
        ? scannedData.amount
        : Number(scannedData.amount);
    const scannedDate = toDateInputValue(scannedData.date);
    const scannedMerchant =
      typeof scannedData.merchant === "string"
        ? scannedData.merchant.trim()
        : "";
    const scannedNote =
      typeof scannedData.note === "string" ? scannedData.note.trim() : "";
    const scannedPaymentMethod = scannedData.paymentMethod;
    const scannedCategoryId =
      scannedData.categoryId ?? scannedData.category?.id ?? null;
    const scannedCategoryName =
      scannedData.category?.name ?? scannedData.categoryName ?? null;
    const scannedBudgetId =
      scannedData.budget?.id ?? scannedData.budgetId ?? null;

    let matchedBudget = null as BudgetApiItem | null;

    if (scannedBudgetId) {
      matchedBudget =
        budgets.find((budget) => budget.id === scannedBudgetId) ?? null;
    }

    if (!matchedBudget) {
      matchedBudget = findMatchingBudget(
        scannedCategoryId,
        scannedCategoryName,
      );
    }
    ...
    setFormState((current) => ({
      ...current,
      amount:
        Number.isFinite(scannedAmount) && scannedAmount > 0
            ...
            <div className="relative">
              ...
              >
                ...
                {budgets.map((budget) => {
                  ...
                  return (
                    <option key={budget.id} value={budget.id} disabled={isFull}>
          ...
          <div>
            ...
            {remaining !== null && (
              ...
              >
                {wouldExceed ? (
                  <>
                    <AlertTriangle size={11} /> Exceeds remaining budget of Rs.
              ...
              <div className="relative">
                <select
                  ...
                  className={`w-full appearance-none rounded-xl border-2 border-gray-200 bg-white px-3 py-2.5 pr-8 text-xs outline-none transition-all focus:border-[#00C950] ${
                    formState.paymentMethod ? "text-gray-700" : "text-gray-400"
                  }`}
                >
