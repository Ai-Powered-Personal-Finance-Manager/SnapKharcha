...
import { useDeleteBudget } from "@/src/features/budgets/api";
...
const lightenColor = (hex: string, whitePercent: number = 80) => {
  ...
  const red = Math.round(
    ((colorValue >> 16) & 255) * colorWeight + 255 * whiteWeight,
  );
  const green = Math.round(
    ((colorValue >> 8) & 255) * colorWeight + 255 * whiteWeight,
  );
    ...
    <div className="group rounded-2xl border border-gray-100 bg-white p-5 transition-all duration-200 hover:shadow-md hover:shadow-gray-100">
      ...
      <div className="mb-2 flex items-end justify-between">
        <div>
          ...
          <p className="text-[11px] text-gray-400">
            of Rs. {budgetData.amount.toLocaleString()} budget
          </p>
        </div>
        <p
          className={`text-sm font-bold font-mono ${over ? "text-red-500" : "text-gray-400"}`}
        >
          {pct}%
        </p>
      </div>
      ...
      <div
        className={`flex items-center gap-1.5 text-[11px] ${over ? "text-red-500" : "text-gray-400"}`}
      >
