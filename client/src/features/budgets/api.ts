...
import type {
  BudgetApiItem,
  BudgetApiResponse,
} from "@/src/features/budgets/types";
...
export const createBudget = async (
  ...
  const response = await clientAPI.post<BudgetMutationResponse>(
    "/budget",
    payload,
  );
  return response.data;
...
export const updateBudget = async ({
  ...
  const response = await clientAPI.patch<BudgetMutationResponse>(
    `/budget/${id}`,
    payload,
  );
  return response.data;
...
export const deleteBudget = async (
  ...
  const response = await clientAPI.delete<DeleteBudgetResponse>(
    `/budget/${id}`,
  );
  return response.data;
};
...
export const useCreateBudget = () => {
  ...
  const { invalidate } = useInvalidates();
  ...
  >({
    ...
    onSuccess: async (response) => {
      ...
      invalidate([CONFIG.REVALIDATE.DASHBOARD]);
    },
    ...
    onSuccess: async (response, variables) => {
      ...
      await Promise.all([
        ...
        queryClient.invalidateQueries({
          queryKey: budgetDetailQueryKey(variables.id),
        }),
      ]);
      invalidate([CONFIG.REVALIDATE.DASHBOARD]);
    },
