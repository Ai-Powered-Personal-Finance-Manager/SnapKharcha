...
import { useDeleteExpense, useGetExpenses, useCreateExpense, useUpdateExpense } from "@/src/hooks/expenses/useExpenses";
import { useGetBudgets } from "@/src/hooks/budgets/useBudgets";
import type { ExpenseApiItem, ExpenseFormValues } from "@/src/types/expense";
...
export function ExpensesPage() {
    ...
    const filteredExpenses = useMemo(() => {
        return expenses.filter((expense) => {
            const budgetMatch = activeBudgetFilter === "all" || expense.budgetId === activeBudgetFilter;
            ...
            const searchMatch = searchText.length === 0 || searchable.includes(searchText);
        ...
        <>
            <div className="space-y-6">
                    ...
                    <div className="flex items-start gap-3 rounded-2xl border border-amber-100 bg-amber-50 px-4 py-3.5">
                        ...
                        <div className="flex-1">
                            <p className="text-xs leading-relaxed text-amber-700">
                                <span className="font-semibold">{nearLimitBudgets.map((budget) => budget.name).join(", ")}</span>{" "}
                                {nearLimitBudgets.length === 1 ? "is" : "are"} near the limit. New expenses in {nearLimitBudgets.length === 1 ? "this budget" : "these budgets"} will be flagged.
                            </p>
                        </div>
                    </div>
                )} */}
                ...
                <div className="flex flex-wrap items-center gap-2">
                    ...
                    {budgets.map((budget) => {
                        const expenseCount = expenses.filter((expense) => expense.budgetId === budget.id).length;
                        ...
                        return (
                            <button
                                key={budget.id}
                                type="button"
                                onClick={() => setActiveBudgetFilter(isActive ? "all" : budget.id)}
                                className={`flex items-center gap-1.5 rounded-xl px-3 py-1.5 text-xs font-semibold transition-all ${
                                    isActive ? "text-white shadow-sm" : "border border-gray-200 bg-white text-gray-500 hover:border-gray-300"
                                }`}
                                style={isActive ? { backgroundColor: categoryColor } : {}}
                            >
                                <span>{budget.name}</span>
                                <span className={`rounded-full px-1.5 py-0.5 text-[10px] ${isActive ? "bg-white/20 text-white" : "bg-gray-100 text-gray-400"}`}>
                                    {expenseCount}
                                </span>
                            </button>
                        );
                    ...
                    <div className="hidden space-y-5 xl:flex xl:flex-col">
                        <div className="overflow-hidden rounded-2xl border border-gray-100 bg-white">
                            <div className="flex items-center justify-between border-b border-gray-50 px-4 py-4">
                                <div>
                                    <h3 className="text-sm font-semibold text-gray-900">Budget Health</h3>
                                    <p className="text-[11px] text-gray-400">Click to filter expenses</p>
                                </div>
                                <ArrowDownLeft size={14} className="text-gray-300" />
                            </div>
                            <div className="space-y-2 p-3">
                                {budgets.slice(0, 5).map((budget) => {
                                    const progress = budget.amount > 0 ? Math.min(Math.round(((budget.spendAmount ?? 0) / budget.amount) * 100), 100) : 0;
                                    ...
                                    return (
                                        <button
                                            key={budget.id}
                                            type="button"
                                            onClick={() => setActiveBudgetFilter(activeBudgetFilter === budget.id ? "all" : budget.id)}
                                        ...
                                        >
                                            ...
                                            >
                                                <TrendingDown size={15} style={{ color: categoryColor }} />
                                            </div>
                                            <div className="min-w-0 flex-1">
                                                <div className="mb-1 flex items-center justify-between">
                                                    <p className="truncate text-xs font-semibold text-gray-700">{budget.name}</p>
                                                    <p className={`ml-2 shrink-0 font-mono text-[10px] font-bold ${isFull ? "text-red-500" : isNear ? "text-amber-600" : "text-gray-500"}`}>
                                                        {progress}%
                                                    </p>
                                                </div>
                                                <div className="h-1.5 overflow-hidden rounded-full bg-gray-100">
                                                    <div
                                                        ...
                                                        style={{ width: `${progress}%`, backgroundColor: isFull ? "#ef4444" : isNear ? "#f59e0b" : categoryColor }}
                                                    />
                                                </div>
                                                <div className="mt-1 flex justify-between">
                                                    <p className="text-[10px] font-mono text-gray-400">{formatCurrency(budget.spendAmount ?? 0)}</p>
                                                    <p className="text-[10px] font-mono text-gray-400">{formatCurrency(budget.amount)}</p>
                                                </div>
                                            </div>
                                        </button>
                                    );
            ...
            <ExpenseDeleteDialog
                ...
                expenseLabel={deletingExpense?.merchant || deletingExpense?.note || "this expense"}
