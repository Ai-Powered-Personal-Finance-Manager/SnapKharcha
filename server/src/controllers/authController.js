...
export const register = async (req, res, next) => {
    try {
        ...
        res.status(201).json({ success: true, message: 'User registered successfully', email: user.email });
    } catch (error) {
...
export const resetForgotPassword = async (req, res, next) => {
    try {
        ...
        const isSame = await bcrypt.compare(newPassword, user.password);
        if (isSame) {
            return next(new AppError('New password cannot be the same as previous password', 400));
        }
